...
/**LENA-START**/
static const char* 
	set_lena_dns_lookup_interval(cmd_parms *param, void *dummy, const char *arg) 
{
	int ttl = atoi(arg);
	if(ttl < 0) {
		return "ProxyLENAttl must be over  0";
	}
	lena_dns_ttl = apr_time_from_sec(ttl);
...
static const char* 
	set_lena_dns_trace(cmd_parms *param, void *dummy, const char *arg) 
{
	if(!strcasecmp(arg,"on")) {
		lena_dns_ttl_trace = 1;
	}else {
		lena_dns_ttl_trace = 0;
	}
...
static const char* 
	set_lena_dns_trace_detail(cmd_parms *param, void *dummy, const char *arg) 
{
	if(!strcasecmp(arg,"on")) {
		lena_dns_ttl_trace_detail = 1;
	}else {
		lena_dns_ttl_trace_detail = 0;
	}
...
static const command_rec proxy_cmds[] =
{
    ...
    AP_INIT_FLAG("Proxy100Continue", forward_100_continue, NULL, RSRC_CONF|ACCESS_CONF,
     ...
     /**LENA-START**/
    AP_INIT_TAKE1("ProxyLENATTL", set_lena_dns_lookup_interval, NULL, RSRC_CONF,
     "Set the timeout (in seconds) for a lena dns lookup interval "),
    AP_INIT_TAKE1("ProxyLENATTLTrace", set_lena_dns_trace, NULL, RSRC_CONF,
     "Set the flag for  lena dns lookup change event "),
    AP_INIT_TAKE1("ProxyLENATTLTraceDetail", set_lena_dns_trace_detail, NULL, RSRC_CONF,
     "Set the flag for  lena dns lookup change event "),
     /**LENA-END**/
    {NULL}
};
...
/**LENA-START**/
static void  *APR_THREAD_FUNC  lena_lookup_func(apr_thread_t *th,void *data) {
	server_rec* s = (server_rec*)data;
	apr_status_t rv; 
	for(;;) {
		apr_hash_index_t *hi;
		for (hi = apr_hash_first(NULL,lena_lookup_target ); hi; hi = apr_hash_next(hi)) {
			apr_sockaddr_t *sa;
			char *key;
			char *temp_key;
			char *org_ip; 
			...
			apr_hash_this(hi, (const void**)&key, NULL, (void**)&org_ip);
			temp_key = apr_pstrdup(lena_pool,key);
			hostname = apr_strtok(temp_key,":",&next);
			org_cport = apr_strtok(NULL,":",&next);
			if(org_cport) {
				org_port = atoi(org_cport);
			}
			if(lena_dns_ttl_trace_detail) {
				ap_log_error(APLOG_MARK,APLOG_NOTICE,0,s,"LENA dns lookup : hostname: %s ip:%s  port:%d",hostname,org_ip,org_port);
			}
				
			rv = apr_sockaddr_info_get(&sa, hostname, 
									   APR_UNSPEC,org_port,0,lena_pool);
			if(rv != APR_SUCCESS) {
				ap_log_error(APLOG_MARK,APLOG_ERR,0,s,"%s ","LENA dns lookup thread : cannot get address info");
				apr_sleep(lena_dns_ttl);
				continue;
			}

			rv = apr_sockaddr_ip_get(&new_ip,sa);
			if(rv != APR_SUCCESS) {
				ap_log_error(APLOG_MARK,APLOG_ERR,0,s,"%s ","LENA dns lookup thread : cannot get ip from address info");
				apr_sleep(lena_dns_ttl);
				continue;
			}

			if(strcmp(org_ip,new_ip) ||  (org_port != sa->port) ) {
				lena_dns_modified_time = apr_time_now();
				/*
				 * old entry is removed (set value = NULL) 
				 * new entry is added by determine_connection in proxy_util.
				*/
				apr_hash_set(lena_lookup_target,key,APR_HASH_KEY_STRING,NULL);
				apr_sleep(lena_dns_ttl);
				if(lena_dns_ttl_trace) {
					ap_log_error(APLOG_MARK,APLOG_NOTICE,0,s,
							"LENA dns lookup -> sockaddr changed : hostname: %s ip:%s -> %s  port:%d -> %d"
														,hostname,org_ip,new_ip,org_port,sa->port);
				}
			}
...
static void init_lena_dns_lookup_thread(apr_pool_t *pool,server_rec *s) {
	apr_status_t rv;
	apr_thread_t *th;
	apr_pool_create(&lena_pool,pool);
	/* lena_lookup_target = apr_table_make(pool,100); */
	lena_lookup_target = apr_hash_make(lena_pool);
	lena_dns_modified_time = apr_time_now();
	rv = apr_thread_create(&th,NULL,lena_lookup_func,s,lena_pool);
	if(rv != APR_SUCCESS) {
		ap_log_error(APLOG_MARK,APLOG_ERR,0,s,"init lena dns lookup thread fail.");
	}
	
	apr_thread_detach(th);
}
/**LENA-END**/
...
static void child_init(apr_pool_t *p, server_rec *s)
{
	...
	if(s) {
		if(lena_dns_ttl > 0) {
			init_lena_dns_lookup_thread(p,s);
		}
	}
	/**LENA-END**/
