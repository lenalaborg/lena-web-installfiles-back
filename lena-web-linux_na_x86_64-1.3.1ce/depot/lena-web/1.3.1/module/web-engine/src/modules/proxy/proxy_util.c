...
PROXY_DECLARE(apr_status_t) ap_proxy_initialize_worker(proxy_worker *worker, server_rec *s, apr_pool_t *p)
{
    ...
    if (worker->s->status & PROXY_WORKER_INITIALIZED) {
        ...
        /**LENA-START**/
		if(lena_dns_ttl > 0) {
			worker->s->lena_dns_modified_time = lena_dns_modified_time;
		}
		/**LENA-END**/
    }
                              ...
                              int server_portstr_size)
{
    ...
    if (uds_path) {
        ...
        if (will_reuse) {
            ...
            /**LENA-START**/
			if(lena_dns_ttl > 0) {
				if(worker->s->lena_dns_modified_time < lena_dns_modified_time) {
					...
					err = apr_sockaddr_info_get(&(worker->cp->addr),
                                            ...
                                            worker->cp->pool);
					if (err != APR_SUCCESS) {
						ap_log_rerror(APLOG_MARK, APLOG_ERR, 0, r, "LENA Proxy: DNS lookup failure for %s.",conn->hostname);

					}
					if(lena_dns_ttl_trace) {
								ap_log_rerror(APLOG_MARK, APLOG_NOTICE, 0, r, 
										"LENA Proxy: Address of %s changed.",conn->hostname);
					}
            ...
            if (!worker->cp->addr) {
            	...
            	/**LENA-START**/
				/* dns lookup check executed worker level. so connection of workes's connection pool may be has previous connection info. */
				/* in that case , below code executed.*/
				if(conn->addr && !apr_sockaddr_equal(conn->addr,worker->cp->addr)) {
					conn->addr = worker->cp->addr;
					conn->close = 1;
				} else {
					conn->addr = worker->cp->addr;
            ...
            /**LENA-START**/
			if(lena_dns_ttl > 0) {
				if(apr_sockaddr_ip_get(&ipaddr,worker->cp->addr) == APR_SUCCESS) {
					char* cport = apr_itoa(lena_pool,conn->port);
					char* target_key = apr_pstrcat(lena_pool,conn->hostname,":",cport,NULL);
					if(apr_hash_get(lena_lookup_target,target_key,APR_HASH_KEY_STRING) == NULL) {
						apr_hash_set(lena_lookup_target,apr_pstrdup(lena_pool,target_key),
										APR_HASH_KEY_STRING,apr_pstrdup(lena_pool,ipaddr));
						if(lena_dns_ttl_trace) {
							ap_log_rerror(APLOG_MARK, APLOG_NOTICE, 0, r, "LENA Proxy: DNS lookup failure for %s.",conn->hostname);
						}
					}
				}
			}
